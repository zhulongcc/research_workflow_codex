...
| 每篇 `translation_zh.md` | 摘要、Highlights、引言、相关工作、方法、不足、未来展望的独立译文及总结 |
| 每篇 `qa.md` | 六问完整问题、答案与依据，空白个人区 |
...
note 是精读后相对概括性的解读，不能替代 translation_zh；后者七部分各有明确的中文翻译小节和实际正文。
摘要、Introduction、Related Work 与 Method 方法部分全部正文逐段译完；方法须完整覆盖全章及所有小节；
不足和未来展望翻译作者全部相关论述。没有独立标题须查找等价/分散内容；确无论述在译文区声明并给核查范围，
摘要、Highlights 和方法不能用缺省声明免除。禁止以 note 跳转、概要、总结、术语表或覆盖表替代译文，不为去重删除与 note 重叠的应译段落。
材料缺失不等于原文不存在，仍属未完成；独立核验须逐段回查原文覆盖及缺省声明，完整记录只入唯一内部状态。
