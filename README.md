...
摘要、Introduction 和 Related Work 全部正文逐段译完，方法完整翻译核心表示、目标与流程的关键段落，不足和展望译完作者全部相关论述。各节的中文翻译与 Agent 总结分开；note 保持相对概括性的精读解读，不能用“见 note”、概要或表格代替应译内容，也不能为了去重删减译文。没有独立章节标题时查找等价或分散论述；确无内容须注明核查范围，材料缺失不能冒充不存在。自检和独立核验都须逐段回查原文覆盖。
